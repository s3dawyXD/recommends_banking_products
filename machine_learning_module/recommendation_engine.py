...
class RecommendationEngine():
	...
	def getTarget(self,row):
		...
		for col in self.target_cols:
			if str(row[col]).strip() in ['', 'NA', 'nan']:
				target = 0
			else:
	...
	def getCustSeniority(self, row):
		...
		missing_value = 0.
		cust_seniority = str(row['antiguedad']).strip()
		if cust_seniority in['NA' , '', 'nan']:
	...
	def processData(self, in_file_name, cust_dict):
		x_vars_list = []
		y_vars_list = []
		for row in csv.DictReader(in_file_name):
			...
			if row['fecha_dato'] in ['2015-05-28', '2016-05-28']:	
				target_list = self.getTarget(row)
				cust_dict[cust_id] =  target_list[:]
				continue

			x_vars = []
			for col in self.cat_cols:
				x_vars.append( self.getIndex(row, col) )
			x_vars.append( self.getAge(row) )
			x_vars.append( self.getCustSeniority(row) )
			x_vars.append( self.getRent(row) )
	...
	def runXGB(self, train_X, train_y, seed_val=0):
		param = {}
	...
	def predict(self, df):
		...
		for _, row in df.iterrows():
			...
			x_vars.append( self.getCustSeniority(row) )
			x_vars.append( self.getRent(row) )
			prev_target_list = self.cust_dict.get(cust_id, [0]*22)
			x_vars_list.append(x_vars + prev_target_list)
		test_X = np.array(x_vars_list)
		xgtest = xgb.DMatrix(test_X)
		preds = self.model.predict(xgtest)
		del x_vars_list, test_X, xgtest
